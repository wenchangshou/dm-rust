...
/// 协议工厂函数类型
type ProtocolFactory = Box<dyn Fn() -> Arc<dyn ProtocolHandler> + Send + Sync>;
...
impl ProtocolRegistry {
    ...
    /// 创建协议处理器实例
    pub fn create(&self, protocol: &str) -> Option<Arc<dyn ProtocolHandler>> {
...
mod tests {
    ...
    fn test_create_protocol() {
        let registry = ProtocolRegistry::new();
        let handler = registry.create("scene_loader");
